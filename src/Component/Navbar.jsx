import { useState } from 'react';
import { Link } from 'react-router-dom';
import Wrapper from "../Component/wrapper";

const Navbar = () => {
  const [activeDropdown, setActiveDropdown] = useState(null);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const toggleDropdown = (menu) => {
    setActiveDropdown((prev) => (prev === menu ? null : menu));
  };

  const toggleMobileMenu = () => {
    setIsMobileMenuOpen((prev) => !prev);
  };

  // Dropdown menu items
  const menuItems = {
    foundation: [
      { label: 'About Us', link: '/about' },
      { label: 'Our Team', link: '/team' },
      { label: 'History', link: '/history' },
    ],
    services: [
      { label: 'Consulting', link: '/consulting' },
      { label: 'Training', link: '/training' },
      { label: 'Method Development', link: '/development' },
    ],
    contact: [
      { label: 'Email Us', link: '/email' },
      { label: 'Find Us', link: '/location' },
      { label: 'Support', link: '/support' },
    ],
    application: [
      { label: 'Note 1', link: '/note1' },
      { label: 'Note 2', link: '/note2' },
      { label: 'Note 3', link: '/note3' },
    ],
  };

  return (
    <header className="bg-black text-white font-[Poppins sans-serif] drop-shadow-xl sticky top-0 left-0 w-full z-50">
      <Wrapper>
        <nav className="flex justify-between items-center px-4 lg:px-16 py-5">
          {/* Logo */}
          <div>
            <Link to="/">
              <p className="text-2xl font-bold sm:text-xl">
                labwox<span className="text-red-600 text-3xl font-extrabold sm:text-2xl">.</span>
              </p>
            </Link>
          </div>

          {/* Hamburger Menu for Mobile */}
          <div className="lg:hidden">
            <button
              onClick={toggleMobileMenu}
              className="text-white focus:outline-none"
              aria-label="Toggle Menu"
            >
              <span className="text-3xl">☰</span>
            </button>
          </div>

          {/* Navigation Links */}
          <div
            className={`lg:flex flex-col lg:flex-row lg:gap-8 items-center absolute lg:static bg-black w-full lg:w-auto transition-all duration-300 ease-in-out ${
              isMobileMenuOpen ? 'top-16 left-0' : 'top-[-500px] left-0'
            }`}
          >
            <ul className="flex flex-col lg:flex-row gap-6 text-sm font-medium lg:items-center px-4 lg:px-0">
              {Object.keys(menuItems).map((menu) => (
                <li key={menu} className="relative group">
                  {/* Dropdown Toggle */}
                  <button
                    className="flex items-center capitalize focus:outline-none"
                    onClick={() => toggleDropdown(menu)}
                  >
                    {menu}
                    <span
                      className={`ml-2 transform transition-transform ${
                        activeDropdown === menu ? 'rotate-180' : ''
                      }`}
                    >
                      ▼
                    </span>
                  </button>
                  {/* Dropdown Menu */}
                  {activeDropdown === menu && (
                    <ul className="absolute top-full left-0 bg-black text-white mt-2 py-2 shadow-lg rounded-lg z-50 w-48 lg:w-auto">
                      {menuItems[menu].map((item) => (
                        <li key={item.label} className="py-2 px-4 hover:bg-gray-800 rounded-lg">
                          <Link to={item.link}>{item.label}</Link>
                        </li>
                      ))}
                    </ul>
                  )}
                </li>
              ))}
            </ul>
          </div>
        </nav>
      </Wrapper>
    </header>
  );
};

export default Navbar;
